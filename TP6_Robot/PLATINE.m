...
while continuer
    ...
    disp ('Pour optimiser une trajectoire 100 fois, taper 8');
    disp ('*')
    ...
    switch reponse
        ...
        case 7 % Deformation de la trajectoire
            ...
            %
          case 8 
            %Optimisation successives jsuqu'à ce qu'il n'y ai plus de
            %collisions
             cprintf('magenta','Optimisation de trajectoire jusqua 100 fois');
               disp(' ');
            if exist('Deformation_trajectoire','file') == 2
                %
                compteur=0;
                while (sum(T(1).nbrecollision)>0 && compteur < 100)
                    compteur = compteur +1;
                    Deformation_trajectoire; 
                    Simulation5; %Simulation sans affichage
                end
             cprintf('magenta','Fait en %d étapes',compteur);
             disp(' ');
            else
                cprintf('red','Le script Deformation_trajectoire n''existe pas !');
            end
